...
struct BWT_same_Checker{
	...
	bool next(){
		...
		for(;bwt_begin < bwt_end; bwt_begin++){
			...
			//fprintf(stderr, "Begin bwt_sa: \n");
			golbal_offset = idx->bwt_sa(bwt_begin);
			//fprintf(stderr, "END bwt_sa: \n");
			//uint64_t golbal_offset = random() % seq_len;
			if(false)fprintf(stderr, "BEGIN sa_depos: %ld %d %ld\n", golbal_offset, is_rev, idx->seq_len);
			golbal_offset = idx->sa_depos(golbal_offset, is_rev);
			xassert(golbal_offset < idx->seq_len , "golbal_offset is overbound!");
			if(false)fprintf(stderr, "END sa_depos: %ld %d \n", golbal_offset, is_rev);
			//show string:
 			int64_t l = 0;
			if(is_rev){
				int64_t beg = golbal_offset - search_length;
				int64_t end = golbal_offset;
				//fprintf(stderr, "_get_pac: beg %ld end  %ld \n", beg, end);
			...
			if(false){
				for(int i = search_length - 1; i >= 0; i--){
					fprintf(stderr, "%c", "ACGTN"[str[i]]);
				}
				fprintf(stderr, "STR: \n");
				for(int i = search_length - 1; i >= 0; i--){
					fprintf(stderr, "%c", "ACGTN"[old_str[i]]);
				}
				fprintf(stderr, "old_str: \n");
			}
			...
			if(str_same == false){
				bwt_begin++;
				if(false) fprintf(stderr, "str_same false, next break: \n");
...
public:
	...
	int generate_SA_index_from_BWA_SA(int argc, char *argv[]) {
			...
			fprintf(stderr, "BEGIN: [load_all_index]\n");
			hap_idx.load_all_index(wb_dir, NULL, true);
			//loading hap_string size
			std::vector<std::vector<uint32_t>> hap_end_offset_table;
			fprintf(stderr, "BEGIN: [loading hap_string size]，wb_info_size is %d\n", hap_idx.wb_info_size);
			{
			...
			fprintf(stderr, "END: [loading hap_string size]\n");

			BWT_IDX_loader bwt_idx;
			//load BWT
			fprintf(stderr, "BEGIN: [load_BWT_index],may be slow\n");
			bwt_idx.restore_idx(bwa_idx_dir, true, true, true, true);
			fprintf(stderr, "END: [load_BWT_index]\n");
			...
			while (ck.next()) {
				...
				for(auto &r : ck.rst){
					...
					if(false && local_offset == 1200 &&  r.is_rev == true)
					{
						ck.show_c_same_bwt();
						fprintf(stderr, " ");
					}
...
//					}
	...
	void sa_search_core(uint64_t bwt_bg, uint64_t bwt_ed, int64_t &sa_idx_start, int64_t &sa_idx_end, int64_t max_load)
	{
		sa_idx_end = -1; sa_idx_start = 0;
			if(bwt_bg > bwt_ed)
				return;
	...
	//void sa_search_core(uint64_t bwt_bg, uint64_t bwt_ed, int64_t &sa_idx_start, int64_t &sa_idx_end);
	void sa_search(uint64_t bwt_bg, uint64_t bwt_ed, std::vector<uint64_t> &SA_pos_result, bool ONLY_REF, int64_t max_load){
		SA_pos_result.clear();
		if(bwt_bg > bwt_ed)
			return;
		int64_t sa_idx_start; int64_t sa_idx_end;
		sa_search_core(bwt_bg, bwt_ed, sa_idx_start, sa_idx_end, max_load);
		if(max_load < sa_idx_end - sa_idx_start){
			SA_pos_result.clear(); return;
		}
		for(int64_t idx=sa_idx_start; idx <=sa_idx_end; idx ++){
			uint64_t data = SA_data[idx] >> SA_IDX_OFFSET;
			if(!ONLY_REF || (((data >> 1 ) & POS_MASK) > WB_POS_MAX))
				SA_pos_result.emplace_back(data);
		}
	}

	uint64_t* get_SA_data(){
		return SA_data;
	}

	void sa_search_M1(uint64_t bwt_bg, uint64_t bwt_ed, std::vector<uint64_t> &SA_pos_result, bool ONLY_REF, int64_t max_load)
	{
		SA_pos_result.clear();
		if(bwt_bg > bwt_ed)
			return;
	...
	bool is_right_haplotype(std::vector<std::vector<uint32_t>> &hap_end_offset_table,
		...
		for(uint i = 0; i < cur_wb_hap_end_offset_list.size(); i++){
			...
			uint hap_ed = cur_wb_hap_end_offset_list[i]-1;//299,599,899~~~~
			if(false)
