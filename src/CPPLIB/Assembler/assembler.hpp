...
struct AssemblyReadAction{
	...
	void print(FILE *output) const {
		fprintf(output,
			"[CP: %d RID:%d %c ",
			position_in_contig, read_ID, isAdd?'+':'-');
...
struct AssemblyContig {
	...
	void debug_print(FILE *output) const {
		...
		fprintf(output,
				...
				"ending_reason: [%s @left edge; %s @Right edge]"
				"seq:\n",
				seq.size(), seedReadCount, supportReads.size(), (ending_reason[0]==0)?"Stop normally":"Repeat", (ending_reason[1]==0)?"Stop normally":"Repeat");

		const char *seq_ = seq.c_str();
		static const unsigned rowSize(100);
		static const unsigned sectionSize(50);
		assert(nullptr != seq_);
