...
void load_from_file_to_json(const char * json_fn,  neb::CJsonObject &oJson){
    std::ifstream fin(json_fn);
    if (fin.good())
    {
        std::stringstream ssContent;
        ssContent << fin.rdbuf();
        std::string s = ssContent.str();

        if (!oJson.Parse(ssContent.str()))
            fprintf(stderr, "parse json error\n");
        //std::cerr << oJson.ToString() << std::endl;

        fin.close();
    }
}

void store_string_int_map_to_json_uint64_t(const char * map_title,  std::map<std::string, uint64_t> &the_map, neb::CJsonObject &j){
	j.AddEmptySubObject(map_title);
	for(std::map<std::string, uint64_t>::iterator it = the_map.begin();
			it != the_map.end(); it++){
		j[map_title].Add(it->first.c_str(), it->second);
	}
}

void store_string_int_map_to_json_uint32_t_(const char * map_title,  std::map<std::string, uint32_t> &the_map, neb::CJsonObject &j){
	j.AddEmptySubObject(map_title);
	for(std::map<std::string, uint32_t>::iterator it = the_map.begin();
			it != the_map.end(); it++){
		j[map_title].Add(it->first.c_str(), it->second);
	}
}
//json to map, used for uint32_t or uint64_t
void load_string_int_map_from_json_uint32_t(std::map<std::string, uint32_t> &the_map, neb::CJsonObject &j){
	the_map.clear(); uint32_t v = 0; std::string key;
	while(j.GetKey(key)) {		j.Get(key, v);		the_map[key] = v;	}
}

void load_string_int_map_from_json_uint64_t(std::map<std::string, uint64_t> &the_map, neb::CJsonObject &j){
	the_map.clear(); uint64_t v = 0; std::string key;
	while(j.GetKey(key)) {		j.Get(key, v);		the_map[key] = v;	}
}
