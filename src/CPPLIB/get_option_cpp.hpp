 ...
 */
 ...
 struct option_item{
	 ...
	 void add_alias(const char * long_option_alias_){
		 long_option_aliases.emplace_back(long_option_alias_);
	 }
 
	 std::string long_option;
	 std::vector<std::string> long_option_aliases;
	 char short_option;
	 ...
	 void show_current_value(FILE * output){
		 ...
		 if(has_arg_pointer){
			 ...
			 else if(arg_type == ARG_TYPE::STR){
				 const char *value = ((const char **)arg_p)[0];
				 fprintf(output, "=%s", value == NULL ? "" : value);
			 }
 ...
 struct options_list{
	 ...
	 std::vector<std::string> long_option_alias_names;
	 ...
	 void add_alias_back(const char * long_option_alias_){
		 l.back().add_alias(long_option_alias_);
	 }
	 ...
	 void parse(){
		 ...
		 long_option.clear();
		 long_option_alias_names.clear();
		 long_option_alias_names.reserve(l.size());
		 conflict_map.clear();
		 for(uint32_t i = 0; i < l.size(); i++){
			 ...
			 if(l[i].long_option.find('_') != std::string::npos){
				 const int alias_has_arg = c_l.has_arg;
				 const int alias_val = c_l.val;
				 long_option_alias_names.push_back(l[i].long_option);
				 std::replace(long_option_alias_names.back().begin(),
						 long_option_alias_names.back().end(), '_', '-');
				 long_option.emplace_back();
				 auto &alias_l = long_option.back();
				 alias_l.name = long_option_alias_names.back().c_str();
				 alias_l.has_arg = alias_has_arg;
				 alias_l.flag = NULL;
				 alias_l.val = alias_val;
			 }
			 for(auto &alias_name : l[i].long_option_aliases){
				 long_option.emplace_back();
				 auto &alias_l = long_option.back();
				 alias_l.name = alias_name.c_str();
				 alias_l.has_arg = c_l.has_arg;
				 alias_l.flag = NULL;
				 alias_l.val = c_l.val;
			 }
			 //set default value
