...
uint64_t get_random_seed_by_time()
{
    struct timeval tp;
    gettimeofday(&tp, NULL);
...
inline void simple_rand(uint64_t *rand_next) {
    *rand_next = ((*rand_next)>>2) * 1103515245 + 48561;
}
...
void ksort_stable_step(void *base, size_t nmemb, size_t size, compar_fn cmp, size_t step)
{
...
char* strcmb(char* to, const char *pre, const char *suf)
{
    strcpy(to, pre);
    strcat(to, suf);
    return to;
